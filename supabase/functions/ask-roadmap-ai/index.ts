...
const PERSONA_LABELS = {
  student: 'Student', employee: 'Employee / Professional',
  entrepreneur: 'Entrepreneur', smb_owner: 'Small Business Owner',
  ceo_executive: 'CEO / Executive', government: 'Government Body',
}
const FALLBACK_SUGGESTIONS = [
  { label: 'Am I on the right track?',      prefill: 'Am I on the right track with this roadmap for my situation?' },
  { label: 'What should I do first?',        prefill: 'What is the single most important thing I should do this week?' },
  { label: 'How realistic is my timeline?', prefill: 'Be honest — how realistic is my timeline?' },
  { label: 'I have less budget now',         prefill: 'I have significantly less budget now. What changes to the roadmap?' },
]
...
function buildRoadmapContext(roadmap, nodes) {
  ...
  const emotional  = nodes.filter(n => n.node_type === 'emotional')
  const personaLabel = PERSONA_LABELS[roadmap.persona] ?? roadmap.persona ?? 'General'
  const phaseLines = phases.map(p => {
    const pMilestones = milestones.filter(m => m.parent_id === p.id)
    const mLines = pMilestones.map(m => {
      const tasks = nodes.filter(n => n.node_type === 'task' && n.parent_id === m.id)
      return `    Milestone: ${m.title}${m.timeline_week_start ? ` (Week ${m.timeline_week_start}-${m.timeline_week_end})` : ''}\n${tasks.map(t => `      - ${t.title}`).join('\n')}`
    }).join('\n')
    return `  Phase: ${p.title} (${p.duration_label ?? ''})\n${mLines}`
  }).join('\n')
  const decisionLines = decisions.map(d => `  Decision: ${d.title}${d.description ? ' — ' + String(d.description).slice(0, 100) : ''}`).join('\n')
  const emotionalLines = emotional.map(e => `  Emotional: ${e.emotional_tag ?? e.title}${e.emotional_note ? ' — ' + String(e.emotional_note).slice(0, 80) : ''}`).join('\n')
  return `ROADMAP: "${roadmap.title}"\nPersona: ${personaLabel}\nGoal: ${roadmap.goal_input}\nDuration: ${roadmap.total_weeks} weeks | Difficulty: ${roadmap.difficulty}\nContext: ${roadmap.context_summary ?? 'N/A'}\nSuccess vision: ${roadmap.success_vision ?? 'N/A'}\n\nSTRUCTURE:\n${phaseLines}\n\nDECISION POINTS:\n${decisionLines || '  None'}\n\nEMOTIONAL CHECKPOINTS:\n${emotionalLines || '  None'}`
}
async function generateSuggestions(apiKey, context, recentHistory) {
  ...
  const prompt = isInitial
    ? `You are generating opening suggestion chips for a user who just opened Ask AI on their personal roadmap.\n\nROADMAP CONTEXT:\n${context}\n\n${historyBlock}\n\nGenerate exactly 4 specific, thoughtful questions this user would genuinely want to ask about THIS specific roadmap — referencing actual phase names, milestones, decision points, or emotional checkpoints.\nUnder 8 words for label, under 20 words for prefill.\n\nReturn ONLY a JSON array, no markdown:\n[{"label": "short label", "prefill": "full question to send"}]`
    : `You are generating follow-up suggestions after a conversation about a personal roadmap.\n\nROADMAP CONTEXT:\n${context}\n\n${historyBlock}\n\nGenerate exactly 4 specific follow-up questions the user would naturally want to ask NEXT. Reference actual phases, milestones, or decision points from this roadmap.\nUnder 8 words for label, under 20 words for prefill.\n\nReturn ONLY a JSON array, no markdown:\n[{"label": "short label", "prefill": "full question to send"}]`
  try {
...
function buildMessagePrompt(userMessage, context, recentHistory) {
  const historyBlock = recentHistory.length > 0
    ? `\nRecent conversation:\n${recentHistory.map(m => `${m.role === 'user' ? 'User' : 'AI'}: ${String(m.content).slice(0, 150)}`).join('\n')}`
    : ''
  return `You are PowerProof's Roadmap AI — a sharp, experienced mentor embedded inside a user's personal roadmap.
...
Rules:
- Reference actual phases, milestones, tasks by name
- Challenge vague thinking, be honest about unrealistic plans
- Under 250 words unless depth is needed
- No padding. Do NOT include "Next action:" or any action text inside the reply field.
...
next_actions rules:
- Exactly 2-3 items
- Each is a complete, standalone action sentence referencing THIS roadmap's actual phases/milestones
- Under 12 words each
...
async function checkRateLimit(db, userId, isByok) {
  try {
    const { data: rl } = await db.rpc('check_and_increment_rate_limit', { p_user_id: userId, p_function_name: 'ask-roadmap-ai', p_calls_per_hour: isByok ? 40 : 20, p_calls_per_day: isByok ? 200 : 100 })
  ...
  } catch (e) { console.error('[ask-roadmap] rate limit:', e) }
  return null
}
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { mode, roadmap_id, session_id, message } = body
    if (!roadmap_id) return new Response(JSON.stringify({ error: 'roadmap_id required' }), { status: 400, headers: corsHeaders })
    const { data: roadmap, error: rmErr } = await db.from('user_roadmaps')
      .select('id, user_id, title, goal_input, persona, context_summary, success_vision, total_weeks, difficulty')
      .eq('id', roadmap_id).eq('user_id', user.id).single()
    if (rmErr || !roadmap) return new Response(JSON.stringify({ error: 'Roadmap not found' }), { status: 404, headers: corsHeaders })
    const { data: nodes } = await db.from('roadmap_nodes')
      .select('id, parent_id, node_type, title, description, emotional_tag, emotional_note, timeline_week_start, timeline_week_end, duration_label')
      .eq('roadmap_id', roadmap_id).order('sort_order', { ascending: true })
    const context = buildRoadmapContext(roadmap, nodes ?? [])
    if (mode === 'history') {
      const { data: sessions } = await db.from('roadmap_chat_sessions').select('id, status, created_at, updated_at, messages').eq('roadmap_id', roadmap_id).eq('user_id', user.id).order('created_at', { ascending: false })
    ...
    if (mode === 'message') {
      ...
      let sessionMessages = []
      const { data: existingSession } = await db.from('roadmap_chat_sessions').select('*').eq('id', session_id).eq('user_id', user.id).maybeSingle()
      if (existingSession) { sessionMessages = existingSession.messages ?? [] }
      else { await db.from('roadmap_chat_sessions').insert({ id: session_id, roadmap_id, user_id: user.id, messages: [], status: 'active' }) }
      ...
      await db.from('roadmap_chat_sessions').update({
    ...
    console.error('[ask-roadmap] error:', err)
