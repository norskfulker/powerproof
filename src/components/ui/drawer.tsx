...
>(({ className, ...props }, ref) => (
  <DrawerPrimitive.Overlay ref={ref} asChild {...props}>
    <motion.div
      className={cn("fixed inset-0 z-[9998] bg-black/60 backdrop-blur-2xl", className)}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
...
>(({ className, children, ...props }, ref) => {
  ...
  const transition = reduceMotion ? { duration: 0.01 } : { type: "spring" as const, stiffness: 520, damping: 44, mass: 0.95 };
